...
void Board::setHighScore() {
    ...
    //Writes file contents to fileHighScore
    readFile >> highScore;
    //If fileHighScore is less than score
    if (highScore<score){
        highScore=score;
        ofstream writeFile;
...
void Board::drawUI(int skin) {
    ...
    drawFilledRect(rightSidePos,0,100,150);
    //Rounding the edges
    drawFilledCircle(rightSidePos,150,10);
    drawFilledRect(rightSidePos,145,100,15);
    drawFilledRect(590,0,10,150);
    //Draw user commands
    setColour(UICol,UICol,UICol);
...
void Board::drawUIText(){
    ...
    string skins="Red skin:R       Green skin:G     Blue skin:B      Rules:I     AI:A";
    drawText(skins,100,650,500,50);
}

//Function to highlight the user input and draw arrow for direction
void Board::drawInput(int input, int skin) {
    int startX=600;
    int startY=300;
    int w=10;
    int h=10;
    ...
    case 1:
        ...
        drawFilledRect(157,635,80,15);
        break;
        //Up input command
    case 2:
        //Drawing box over input
        drawFilledRect(320,635,20,15);
        //Up input arrow
...
void Board::drawState(int state) {
    ...
    if (state==1) {
        ...
        setColour(255,215,0);
        //Drawing win rect
        drawFilledRect(590,200,120,200);
        //Rounding the edges
        drawFilledCircle(590,200,10);
        drawFilledCircle(590,400,10); 
        drawFilledRect(590,190,400,15);
        drawFilledRect(590,390,400,20);    
        drawFilledRect(580,200,15,200);
        //Set text colour
        setColour(0,0,0);
        if (startValue_ !=2){
             drawText("YOU CHEAT",610,235,100,50);
             drawText("BUT",634,255,100,50);
        }
        //Print winner on rect
        drawText("YOU WIN!",615,275,100,50);
    }
    //Else game is over
    else {
        //Set colour
...
void Board::drawRules() {
    ...
    drawText("You can change the game appearance with R,G,B for according skins",110,280,400,50);
    drawText("Start a new game with N",110,300,400,50);
     drawText("Or quit with Q",110,320,400,50);
}
